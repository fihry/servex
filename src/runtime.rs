...
mod tests {
    ...
    fn make_config_with_route(root: &Path, route: Route) -> ServerConfig {
        let mut config = ServerConfig::default();
        config.sessions.inject(&HashMap::from([
    ...
    fn should_keep_alive_respects_http10_and_connection_header() {
        ...
        let req_close =
            make_request_with(Method::Get, "/", "HTTP/1.0", Vec::new(), &[("host", "localhost")]);
    ...
    fn should_keep_alive_honors_global_flag_and_http11_close() {
        ...
        let req =
            make_request_with(Method::Get, "/", "HTTP/1.1", Vec::new(), &[("host", "localhost")]);
    ...
    fn store_upload_sanitizes_multipart_filename() {
        ...
        let request = make_request_with(
            ...
            "HTTP/1.1",
            body,
            &[("host", "localhost"), ("content-type", "multipart/form-data; boundary=abc")],
        );
